...
function drawFloor(floor: Floor, svgElement: SVGElement, zoom: d3.ZoomBehavior<any, any>) {
...
export default class DrawingManager {
  ...
  updateItems(items: Item[]) {
    ...
    const circlesGroupJoin = itemsLayer.selectAll('g')
      .data(itemsArray, (items: Item[]) => String(items[0].mac))
    ...
    const circlesJoin = circlesGroup.selectAll('circle')
      .data(items => items, (item: Item) => String(item.id))
    ...
    const pathJoin = pathLayer.selectAll('path').data(itemsArray, (items: Item[]) => items[0].mac)
    ...
    function showTooltip(itemId: number) {
    ...
    function updateTooltipPosition(itemId: number) {
  ...
  resetTransform(useTransition = true) {
    ...
    const boardElement = this.svgElement.querySelector('.board') as SVGGElement
